...
export function opencodePluginSource(options: OpencodePluginOptions): string {
  if (hookHostPlatform(options.platform) === 'win32') {
    return win32OpencodePluginSource(options)
  }
  return posixOpencodePluginSource(options)
}

function posixOpencodePluginSource(options: OpencodePluginOptions): string {
...
function win32OpencodePluginSource(options: OpencodePluginOptions): string {
  const { adapterPath, timeoutSeconds } = options
...
function runHook(event, envelope) {
  return new Promise((resolve) => {
    let child
    try {
      child = spawn(NODE, [ADAPTER, "hook", event, "--owner", "notifai", "--harness", "opencode"], {
        env: process.env,
        shell: false,
        windowsHide: true,
        stdio: ["pipe", "pipe", "pipe"],
