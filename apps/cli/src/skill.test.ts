...
describe('Notifai agent skill', () => {
  ...
  it('names each immutable session once without asking the agent for its id', () => {
    ...
    expect(send).toMatch(/Managed OpenCode and Orca-managed Claude Code/i)
    expect(send).toMatch(/omit `--session-label` there/i)
    expect(send).toMatch(/Outside those managed sources/i)
    expect(send).toMatch(/Claude Code, Codex,\s+and Cursor/i)
